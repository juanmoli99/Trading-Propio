...
function ingest(symbol, statusCode, tape, timestamp) {
...
const assertions = {
    UTP_HALT_DETECTED: utpHalt.state === 'HALTED' &&
        utpHalt.halted === true,
    UTP_RESUME_DETECTED: utpResume.state === 'NOT_HALTED' &&
        utpResume.halted === false,
    CTA_HALT_DETECTED: ctaHalt.state === 'HALTED' &&
        ctaHalt.halted === true,
    CTA_RESUME_DETECTED: ctaResume.state === 'NOT_HALTED' &&
        ctaResume.halted === false,
    VOLATILITY_PAUSE_DETECTED: volatilityPause.state === 'HALTED' &&
        volatilityPause.halted === true,
    UNKNOWN_CODE_FAILS_CLOSED: unknownCode.state === 'UNKNOWN' &&
        unknownCode.halted === false,
    MISSING_STATUS_IS_UNKNOWN: missing.state === 'UNKNOWN' &&
        missing.status === null,
    HALTED_SYMBOL_REJECTED: haltedRejected,
    UNKNOWN_SYMBOL_REJECTED: unknownRejected,
    RESUMED_SYMBOL_ALLOWED: resumedAllowed,
};
