    ...
    var MarketDataHaltDetectionService = class {
        ...
        detect(symbol) {
            ...
            return {
                ...
                state,
                halted: state === 'HALTED',
                haltedAt: state === 'HALTED'
                    ? new Date(snapshot.status.timestamp)
                    : null,
                haltReason: state === 'HALTED'
        ...
        detectResume(symbol) {
            const snapshot = this.tradingStatusService.getTradingStatus(symbol);
            const previousState = snapshot.previousStatus === null
                ? 'UNKNOWN'
                : this.classify(snapshot.previousStatus);
            const currentState = snapshot.status === null
                ? 'UNKNOWN'
                : this.classify(snapshot.status);
            return {
                symbol: snapshot.symbol,
                resumed: previousState === 'HALTED' &&
        ...
        assertKnownAndNotHalted(symbol) {
            ...
            if (result.state === 'HALTED') {
                ...
                const reasonDescription = reason === null
                    ? 'unknown reason'
                    : this.formatReason(reason);
                throw new Error(`Trading halted for symbol ${result.symbol}: ${reasonDescription}`);
            }
            if (result.state === 'UNKNOWN') {
                throw new Error(`Trading halt state is unknown for symbol ${result.symbol}`);
            }
        }
        extractHaltReason(status) {
            return {
