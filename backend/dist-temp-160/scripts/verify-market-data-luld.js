...
const assertions = {
    LULD_NORMALIZED: first.symbol === 'AAPL' &&
        first.limitUp === 210.5 &&
        first.limitDown === 190.25 &&
        first.indicator === 'B' &&
        first.feed === 'sip',
    LULD_TIMESTAMP_NORMALIZED: first.timestamp.toISOString() === '2026-08-12T20:00:00.000Z',
    LULD_RECEIVED_AT_PRESERVED: first.receivedAt.toISOString() === firstReceivedAt.toISOString(),
    SYMBOL_LOOKUP_NORMALIZED: firstSnapshot.symbol === 'AAPL' &&
        firstSnapshot.luld?.limitUp === 210.5,
    NEWER_LULD_ACCEPTED: finalSnapshot.luld?.limitUp === 211.0 &&
        finalSnapshot.luld?.limitDown === 191.0,
    OLDER_LULD_DOES_NOT_OVERWRITE: finalSnapshot.luld?.indicator === 'B' &&
    ...
    INVALID_MESSAGE_TYPE_REJECTED: invalidMessageTypeRejected,
    INVALID_BANDS_REJECTED: invalidBandsRejected,
};
